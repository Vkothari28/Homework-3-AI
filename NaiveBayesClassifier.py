    ...
    def fit(self, X, y):
        ...
        for row_index in range(0, len(X)):
            ...
            for idx in range(0, len(row)):
                ...
                if int(row[idx]) == 1 and int(y[row_index]) == 0:
                    spam_word += 1
                ...
                if int(row[idx]) == 1 and int(y[row_index]) == 1:
                    ham_word += 1
                    word = self.mapper.get(idx)
...
def calculate_f_score(predicted_labels, actual_labels):
    ...
    for idx in range(0, num_labels):
        ...
        if pred_val == 1:  # Positive values
            if pred_val == actual_val:
                num_tp += 1
            elif pred_val != actual_val:
                num_fp += 1
        elif pred_val == 0:  # Negative values
            if pred_val == actual_val:
                num_tn += 1
            elif pred_val != actual_val:
                num_fn += 1
...
if __name__ == '__main__':
    ...
    filename='dbworld_subjects_stemmed.csv'
    X, y = bayes.feature_label_from_csv(filename)
    ...
    # Convert the csv to numpy array
    file='dbworld_bodies_stemmed.csv'
    X, y = bayes.feature_label_from_csv(file)
